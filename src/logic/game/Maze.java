...
public class Maze {
	...
	public static void algorithm(Celula[][] maze, int posX, int posY) {
		Vector<Celula> disp = new Vector<Celula>();
		Celula ori = maze[posX][posY];
		al2(disp, ori);
	}

	public static void al2(Vector<Celula> disp, Celula ori) {
		ui.printMaze(board);
		System.out.println("TAMANHO= " + disp.size());
		disp.remove(ori);
		Celula ran = new Celula();
		if (!ori.visited) {
			ori.visited = true;
			ori.symbol = 'O';
			...
			if (ori != temp)
				disp.add(temp);
			if (!disp.isEmpty()) {
				ran = disp.get(sorteio(disp.size()));
				al2(disp, ran);
			} else
	...
	public static int sorteio(int number) {
		Random nr = new Random();
		int temp= nr.nextInt(number);
		System.out.println(temp);
		return  temp;
	}

	public static Celula dir(Celula ori, Vector<Celula> disp) {
		if (ori.getX() + 1 < width)
			if (!board[ori.getX() - 1][ori.getY()].visited)
				return board[ori.getX() + 1][ori.getY()];
			else
				disp.remove(board[ori.getX() + 1][ori.getY()]);

		return ori;

	}

	public static Celula esq(Celula ori, Vector<Celula> disp) {
		if (ori.getX() - 1 > width)
			if (!board[ori.getX() + 1][ori.getY()].visited)
				return board[ori.getX() - 1][ori.getY()];
			else
	...
	public static Celula cima(Celula ori, Vector<Celula> disp) {
		if (ori.getY() - 1 < width)
			if (!board[ori.getX()][ori.getY() + 1].visited)
				return board[ori.getX()][ori.getY() - 1];
			else
				disp.remove(board[ori.getX()][ori.getY() - 1]);
		return ori;

	}

	public static Celula baixo(Celula ori, Vector<Celula> disp) {
		if (ori.getY() + 1 < width)
			if (!board[ori.getX()][ori.getY() - 1].visited)
				return board[ori.getX()][ori.getY() + 1];
			else
